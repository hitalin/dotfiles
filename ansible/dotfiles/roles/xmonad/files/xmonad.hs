...
myManageHook = composeAll
  ...
  , className =? "Firefox" <&&>
    foldr1 (<||>) (map (resource =?) ["Dialog", "Toplevel", "Foxmarks"]) --> doFloat
  ]
...
myStartupHook = do
  ...
  io $ setEnv "PATH" (home ++ "/.local/bin:" ++ path)
  io $ setEnv "LANG" "ja_JP.UTF-8"
  io $ setEnv "SSH_AUTH_SOCK" (xdgRuntimeDir ++ "/ssh-agent.socket")
  ...
  spawnOnce "feh --no-fehbg --bg-scale \"$HOME/.wallpaper.jpg\""
  spawnOnce trayer
  spawnOnce "dunst"
  spawnOnce "uim-xim"
  ...
  spawnOnce "ssh-add -l | grep $HOME/.ssh/id_rsa > /dev/null || ssh-add"
    where
